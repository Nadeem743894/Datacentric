...
layout = html.Div(
    children=[
        ...
        dbc.Row(
            dbc.Col(
                html.Button('Button - change length of Line 2', id='button-click', n_clicks=5, style={'font-size' : '24px', 'backgroundColor' : 'green', 'color' : 'white'}), 
...
@app.callback(Output('line-2', 'figure'),
              Input('button-click', 'n_clicks'),
              Input('interval-component', 'n_intervals'))
def update_graph_live(n_clicks, n_intervals):
    df = pd.DataFrame()
    df['Random Signal'] = pd.DataFrame(np.random.randint(0,n_intervals,size=(n_clicks*20, 1)))
    fig = px.line(
        df,
        y='Random Signal',
